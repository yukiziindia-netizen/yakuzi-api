...
async def upload_dataset(background_tasks: BackgroundTasks, file: UploadFile = File(...), custom_name: str = Form("yukizi-custom-bot")):
    ...
    try:
        ...
        # The operation name or model name is returned
        operation = client.tuned_models.create(
            source_model="models/gemini-1.5-flash-001-tuning",
            training_data=types.TuningDataset(
                examples=gemini_file,
            ),
            id=custom_name
        )
        
        # Identify job_id (usually operation.name or operation.model)
        job_id = getattr(operation, 'name', str(operation))
        
        # 3. Start background monitor
        background_tasks.add_task(monitor_tuning_job, job_id, client)
        
        return {
            "message": "Dataset uploaded and tuning job started successfully.",
            "job_id": job_id,
            "status": "TRAINING"
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Failed to start tuning job: {str(e)}")
    finally:
